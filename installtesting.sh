...
set_updater() {
    ...
    # Günlük saat 10:00'da çalışacak şekilde cron ekler
    (crontab -l 2>/dev/null | grep -v "$script_path"; echo "0 10 * * * /bin/bash $script_path --auto-deploy") | crontab -
    echo -e "${GREEN}[✔] Success: Scheduled daily update in crontab.${NC}"
...
uninstall_arkenfox() {
    ...
    if [[ "$un_choice" =~ ^[Yy]$ ]]; then
        ...
        echo -e "${GREEN}[✔] Arkenfox removed and profile restored.${NC}"
    fi
    read -n 1 -s -p "Press any key to return..."
}
